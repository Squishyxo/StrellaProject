<template>
  <div>
    ...
    <Navigation2 v-else/>
        ...
        <h1>Typography</h1>
        <div>
            <p :class="[ editable ? 'edit-border' : '']" id="typography-text" :v-model="text">{{text}}
                <!-- Our logo uses Utah Condensed Bold font. This is a very classic look for our brand. Throughout our websites we use variations of Bureau Borsche and Demna Gvasalia. While using our products there’s always a certain level of luxury and expertise.
                We express this through our font usage. -->
            </p>
        </div>
    </section>
    ...
    <router-link to="/colors" ><button class="next-btn" v-if="!loggedIn">NEXT</button></router-link >
  </div>
</template>
...
export default {
  ...
  // getText function executes whenever the page refreshes
    ...
    methods: {
        getText(){
              fetch('https://s3-project-8f792-default-rtdb.europe-west1.firebasedatabase.app/typographyText.json').then((response) => {
                if (response.ok){
       ...
       editText(){
           ...
           document.getElementById("typography-text").contentEditable = this.editable;
       },
       ...
       saveChanges(){
        ...
        fetch('https://s3-project-8f792-default-rtdb.europe-west1.firebasedatabase.app/typographyText.json', {
            method: 'PUT',
            headers: {
